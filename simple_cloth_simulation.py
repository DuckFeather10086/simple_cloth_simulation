...
def generate_edges(mesh, shape_object, bones, scale, connect_mesh=False, connect_parents=False,
    ...
    for i in range(0,len(bone_chain_edges)-chain_count):
        ...
        if len(bone_chain_edges) >=i+chain_count:
            face = (bone_chain_edges[i][0],bone_chain_edges[i][1],bone_chain_edges[i+chain_count][1],bone_chain_edges[i+chain_count][0]) 
            faces.append(face)
...
class BONE_OT_custom_shape(Operator):
    ...
    bl_description = "Creates a mesh object from the selected bones and generate a simulation cloth object for thease bones"
...
#    bl_context = "bone"
    bl_label = "Simple Colth Simulation"
